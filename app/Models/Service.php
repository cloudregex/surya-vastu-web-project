...
class Service extends Model
{
    ...
    {
        $rules = [
            'columns.service_name' => 'required|string|max:25|unique:services,service_name',
            'columns.service_description' => 'required|string|min:100',
            'columns.service_image' => 'required|image|mimes:jpeg,png,jpg|max:2048|dimensions:width=600,height=500',
        ];

        if ($serviceId) {
            $rules['columns.service_name'] = 'required|string|max:25|unique:services,service_name,' . $serviceId . ',service_id';
            $rules['columns.service_image'] = 'nullable|image|mimes:jpeg,png,jpg|max:2048|dimensions:width=600,height=500';
        }
        return $rules;
    }
    ...
    public static function messages()
    {
        return [
            ...
            'columns.service_name.max' => 'The service name may not exceed 25 characters.',
            ...
            'columns.service_image.dimensions' => 'The image must have dimensions of 600x500 pixels.',
