<div>
    <x-card-comp listRoute="admin.role.list" listButtonName="Go Role List" title="Edit Role">
        <x-slot name="CardBody">
            @if (session()->has('message'))
                <div class="alert alert-success">{{ session('message') }}</div>
            @endif
            <x-input type="text" placeholder="Enter your Role" col="12" name="roleName" value="{{ $roleName }}"
                label="Role Name" />
                ...
                @foreach ($availablePermissions as $key => $group)
                    <div class="col-md-4 pb-3 border p-0">
                        <div class="d-flex justify-content-between bg-primary-gradient p-1">
                            <h6 class="text-white">{{ ucfirst($key) }} Permissions</h6>
